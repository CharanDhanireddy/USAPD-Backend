...
(
   SELECT
      ...
      B.YEAR AS BIGYEAR,
      A.YEAR AS SMALLYEAR,
      ROUND( ( (B.AVGPOLLUTANT - A.AVGPOLLUTANT)* 100 ) / (A.AVGPOLLUTANT), 5 ) AS PERCENTAGE_DIFF 
   FROM
      (
         SELECT
            POL.POLLUTANT_NAME AS POLLUTANT,
            ROUND( AVG(O.ARITHMETIC_MEAN), 6 ) AS AVGPOLLUTANT,
            TO_CHAR(O.DATE_STR2, 'YYYY') YEAR 
         FROM
            VDHAVALESWARAPU.OBSERVATION O NATURAL 
            JOIN
               VDHAVALESWARAPU.POLLUTANT POL 
         WHERE
            (
( O.SITE_CODE IN 
               (
...
SELECT
   ...
   ROUND( (B.POPULATION - A.POPULATION)* 100 / (A.POPULATION), 5 ) POPULATION_PERCENTAGE_DIFF 
FROM
